...
stream.post("/prompt", async (c) => {
  const { clientId, prompt, sessionId, workDir, model, images, permissionMode } = await c.req.json();
  const ok = await handlePrompt(clientId, prompt, sessionId, workDir, model, images, permissionMode);
