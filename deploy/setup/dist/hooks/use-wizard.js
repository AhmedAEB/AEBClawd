...
export function wizardReducer(state, action) {
    switch (action.type) {
        ...
        case "NEXT_STEP":
            return { ...state, step: Math.min(state.step + 1, Step.Done) };
        case "PREV_STEP":
            return { ...state, step: Math.max(state.step - 1, Step.Welcome) };
        case "SET_DOMAIN":
        ...
        case "UPDATE_INSTALL_STEP":
            return {
                ...state,
                installSteps: state.installSteps.map((s) => s.id === action.id
...
export function useWizard() {
    ...
    if (!ctx)
        throw new Error("useWizard must be used within WizardContext.Provider");
    return ctx;
}
