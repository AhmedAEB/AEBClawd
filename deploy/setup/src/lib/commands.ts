...
async function run(cmd: string, args: string[], opts?: Record<string, unknown>): Promise<string> {
  log(`$ ${cmd} ${args.join(" ")}`);
  const result = await execa(cmd, args, { ...opts, reject: false } as any);
...
export async function installCaddy(): Promise<void> {
...
export async function writeCaddyfile(config: WizardConfig): Promise<void> {
  // Hash the password
...
export async function buildFrontend(config: WizardConfig): Promise<void> {
  await run("pnpm", ["--filter", "frontend", "build"], { cwd: config.installDir });
  log("Frontend built");
}

export async function buildServer(config: WizardConfig): Promise<void> {
  await run("pnpm", ["--filter", "server", "build"], { cwd: config.installDir });
  log("Server built");
}

export async function buildBot(config: WizardConfig): Promise<void> {
  await run("pnpm", ["--filter", "bot", "build"], { cwd: config.installDir });
  log("Bot built");
}

export async function installSystemdServices(config: WizardConfig): Promise<void> {
...
export async function configureFirewall(): Promise<void> {
...
export async function startVoiceContainers(config: WizardConfig): Promise<void> {
...
export async function startServices(config: WizardConfig): Promise<void> {
